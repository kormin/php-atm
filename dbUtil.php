...
class PdoDb
{
	...
	*/
	...
	function selectCond($cols, $join, $where) {
		$sql = "SELECT 
			".$cols."
			FROM
			`".$this->dbInfo['table']."`
...
function dbConf($table) {
	$dbInfo['host'] = 'localhost';
	$dbInfo['dbname'] = 'atm';
	$dbInfo['charset'] ='utf8mb4';
	$dbInfo['user'] = 'root';
	$dbInfo['pass'] = '';
	// $dbInfo['host'] = '	hacktivity.comlu.com';
	// $dbInfo['dbname'] = 'a9576819_hacktiv';
	// $dbInfo['charset'] ='utf8mb4';
	// $dbInfo['user'] = 'a9576819_root';
	// $dbInfo['pass'] = 'root1234';
	$dbInfo['driver'] = 'mysql';
	$dbInfo['table'] = $table;
	$dbInfo['path'] = '';
...
function userCol($dbconn) {
	$cols = array("username", "password", "roles_id");
	$len = count($cols);
	$dbconn->setCol($cols, $len);
	$dbconn->setColLen($len);
}
function infoCol($dbconn) {
	// $cols = array("fname","lname","mname","bday","gender","status","spouse","cnum","email","users_id");
	$cols = array("fname","mname","lname","bdate","cnum","mail","gender","status","spouse","users_id");
	$len = count($cols);
	$dbconn->setCol($cols, $len);
	$dbconn->setColLen($len);
}
...
function delId($id) { //delete from db using id
	$cond = "`id`=".$id;
	$db1 = dbConf('user_info');
	$db1->delete($cond);
	$db = dbConf('users');
	$db->delete($cond);
}
